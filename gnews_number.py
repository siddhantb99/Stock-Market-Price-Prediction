...
# execute code for these company sepately ,make companies.csv execulding these companies in companies.csv as these started after 2010
##ADANIPORTS
#BAJAJ-AUTO
#BAJAJFINSV
#BAJFINANCE
...
## for executing all companies

pytrends = TrendReq()
import csv
with open('companies.csv','rt')as f:
    data=csv.reader(f)
    for row in data:
        if(row[0]!='0'):
            print(row[0])
            df=get_daily_trend(pytrends,row[0],"2010-01-04","2020-04-07")
            df.to_csv(row[0]+'.csv')



## for executing one company at a time
'''
pytrends = TrendReq()
df=get_daily_trend(pytrends,"BAJFINANCE","2014-10-04","2020-04-07")
print(df.head())
df.to_csv("BAJFINANCE"+'.csv')
'''
